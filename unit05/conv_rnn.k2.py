...
h = model.fit(train_flat, train_labs, batch_size = 32, epochs=4, validation_data = (test_flat,test_labs), verbose=1)
...
M = concatenate(flats, axis=1)
R = Reshape((8,64))(M)
L = LSTM(units=16, activation='sigmoid', recurrent_activation='hard_sigmoid')(R)
D = Dropout(0.5)(L)
O = Dense(5, activation='sigmoid')(D)
model = Model(inputs=blocks, outputs=[O])
...
n = 100000
h = model.fit(list(train_block_swap[:,:n]), train_labs[:n], batch_size = 32, epochs=10, validation_data = (list(test_block_swap[:,:n]),test_labs[:n]), verbose=1)
...
for xx in range(sx):
    for yy in range(sy):
        con[xx,yy].pcolormesh(W[0][:,:,0,xx].reshape(3,3), cmap=plt.cm.gray) 
